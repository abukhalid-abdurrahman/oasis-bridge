...
{
    [HttpGet]
    public async Task<IActionResult> GetUserRolesAsync([FromQuery] UserRoleFilter filter, CancellationToken cancellationToken)
    ...
    public async Task<IActionResult> CreateUserRoleAsync([FromBody] CreateUserRoleRequest request, CancellationToken cancellationToken)
        => (await userRoleService.CreateUserRoleAsync(request, cancellationToken)).ToActionResult();

    [HttpPut("{id:guid}")]
    [Authorize(Roles = Roles.Admin)]
    public async Task<IActionResult> UpdateUserRoleAsync(Guid id, [FromBody] UpdateUserRoleRequest request, CancellationToken cancellationToken)
